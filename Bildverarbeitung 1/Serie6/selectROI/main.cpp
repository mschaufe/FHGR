...
static void selection_completed() {
    ...
    int width, height;
    // Rectangle get drawn down and right
    if(ptButtonDownPos.x < ptButtonUpPos.x && ptButtonDownPos.y < ptButtonUpPos.y)
    {
        width = ptButtonUpPos.x - ptButtonDownPos.x;
        height = ptButtonUpPos.y - ptButtonDownPos.y;
    }
    // Rectangle get drawn up and left
    if(ptButtonDownPos.x > ptButtonUpPos.x && ptButtonDownPos.y > ptButtonUpPos.y)
    {
        width = ptButtonDownPos.x - ptButtonUpPos.x;
        height = ptButtonDownPos.y - ptButtonUpPos.y;
    }
    // Rectangle get drawn down and left
    if(ptButtonDownPos.x > ptButtonUpPos.x && ptButtonDownPos.y < ptButtonUpPos.y)
    {
        width = ptButtonDownPos.x - ptButtonUpPos.x;
        height = ptButtonUpPos.y - ptButtonDownPos.y;
    }
    // Rectangle get drawn up and right
    if(ptButtonDownPos.x < ptButtonUpPos.x && ptButtonDownPos.y > ptButtonUpPos.y)
    {
        width = ptButtonUpPos.x - ptButtonDownPos.x;
        height = ptButtonDownPos.y - ptButtonUpPos.y;
...
static void mouse_callback(int event, int x, int y, int, void *) {
    ...
    if (lMouseButtonDown == true && lMouseButtonUp == false){
        ...
        rectangle(img2, ptButtonDownPos, pt, Scalar(0, 0, 255));
...
int main(int argc, char* argv[]) {				// accepts command line arguments
    ...
    cv::String filename = "/Users/marc/Dropbox/05_School/04_FHGR/21_Bildverarbeitung1/Arbeitsblaetter/Serie2/BILD1-03-material/moon+ISS.png"; // change to an existing image file
    if (argc > 1) {
        filename = argv[1];
    }
