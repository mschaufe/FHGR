...
static void help(char** argv)
{
    ...
    cout << "\nUsage:\n" << argv[0] << " [image_name -- Default book.jpg]\n" << endl;

    cout << "\nHot keys: \n"
            "\tESC, q - quit the program\n"
            "\tr - change order of points to rotate transformation\n"
            "\tc - delete current point set (new inputs via mouse-click)\n"
            "\ti - change order of points to invers transformation \n"
            "\td - display current point coordinates in console\n"
...
{
    help(argv);
    //CommandLineParser parser(argc, argv, "{@input| book.jpg |}");
    CommandLineParser parser(argc, argv, "{@input| chess.jpg |}");

    string filename = parser.get<string>("@input");
    Mat original_image = imread(filename);
    Mat image;

    auto original_image_cols = (float)original_image.cols;
    auto original_image_rows = (float)original_image.rows;
    roi_corners.emplace_back((float)(original_image_cols / 1.70), (float)(original_image_rows / 4.20));
    roi_corners.emplace_back((float)(original_image.cols / 1.15), (float)(original_image.rows / 3.32));
    roi_corners.emplace_back((float)(original_image.cols / 1.33), (float)(original_image.rows / 1.10));
    roi_corners.emplace_back((float)(original_image.cols / 1.93), (float)(original_image.rows / 1.36));
    ...
    while (!endProgram)
    {
        ...
        if ((c == 'd') | (c == 'D'))
        {
            cout << "Positions (" << roi_corners.size() << " points)" << endl;
            for (int k=0; k < roi_corners.size(); k++)
                cout << (k+1) << ": " << roi_corners[k] << endl;
        }
